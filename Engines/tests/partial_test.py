...
class partial_test(Simulation):
    ...
    def init_fields(self):
        ...
        for i in range(N):
            for j in range(M):
                ...
                phi[i][j] = round(((1.0 - np.tanh((x_p - xpos) / xi)) / 2),1)
                if phi[i][j] > 1.0:
                    phi[i][j] = 1.0
    ...
    def simulation_loop(self):
        ...
        cuda.synchronize()
        if self.time_step_counter % 1 == 0:

            output_folder = '7.7_modified' ##
            os.makedirs(output_folder, exist_ok=True)
            phi = self.fields[0].data
            df = pd.DataFrame(phi)
            filename = os.path.join(output_folder,f'T_bc_phi_step{self.time_step_counter-1}.csv')
            df.to_csv(filename, index=False, header=False)

            c = self.fields[1].data
            df = pd.DataFrame(c)
            filename = os.path.join(output_folder,f'T_bc_c_step{self.time_step_counter-1}.csv')
            df.to_csv(filename, index=False, header=False)

            dcdt = self.fields[12].data
            df = pd.DataFrame(dcdt)
            filename = os.path.join(output_folder, f'T_bc_dcdt_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            cs = self.fields[2].data
            df = pd.DataFrame(cs)
            filename = os.path.join(output_folder, f'T_bc_cs_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            cl = self.fields[3].data
            df = pd.DataFrame(cl)
            filename = os.path.join(output_folder, f'T_bc_cl_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            tarr = self.fields[5].data
            df = pd.DataFrame(tarr)
            filename = os.path.join(output_folder, f'T_bc_tarr_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            theta = self.fields[4].data
            df = pd.DataFrame(theta)
            filename = os.path.join(output_folder, f'T_bc_theta_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)
...
'''
        ...
        M = 2000
        thresh = 0.5 * M
        phi_out = self._fields_out_gpu_device[0]
