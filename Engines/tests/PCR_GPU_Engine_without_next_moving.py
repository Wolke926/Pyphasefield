...
def solveC(fields,params,fields_out):
    ...
    for i in range(starty + 1, phi.shape[0] - 1, stridey):
        for j in range(startx + 1, phi.shape[1] - 1, stridex):
            ...
            Gnbnb = R * T / c_s[i][j]
            Gtinb = 13045.3
            Ds = (1 - c_s[i][j]) * c_s[i][j] * ((1 - c_s[i][j]) * M_Nb + (c_s[i][j]) * (M_ti)) * (
                    Gtiti + Gnbnb - 2 * Gtinb)
            #1st term in (7)
            xplus_s = ((phi_out[i][j] * Ds + phi_out[i+1][j] * Ds) / 2) * ((c_s[i+1][j] - c_s[i][j]) / dx)  # x05
...
#BC
@cuda.jit
def TarrUpdate(fields,params,fields_out,timestep,xoffs):
    startx, starty = cuda.grid(2)
    stridex, stridey = cuda.gridsize(2)
    Tarr = fields[5]
    Tarr_out = fields_out[5]
...
@cuda.jit
def SolvePullBack(fields,params,fields_out,timestep,xoffs):
    phi = fields[0]
    M = phi.shape[0]
    thresh = 0.5 * M
    phi_out = fields_out[0]
    column_index = int(np.floor(thresh))
    maxphi_thresh = np.max(phi_out[:, column_index][1])
    if maxphi_thresh > 0.1:
        PullBack[self._gpu_blocks_per_grid_2D, self._gpu_threads_per_block_2D](self._fields_gpu_device,
                                                                               self.user_data["params_GPU"],
                                                                               self._fields_out_gpu_device)
        xoffs += dx

@cuda.jit
def PullBack(fields,params,fields_out,timestep):
    ...
    for i in range(starty + 1, phi.shape[0] - 1, stridey):
        for j in range(startx+ 1, phi.shape[1]- 1, stridex):
                ...
                if j == M - 1:
                    phi_out[i][M - 1] = phi_out[i][M - 2]
                    c_out[i][M - 1] = c_0
                    c_s_out[i][M - 1] = c_out[i][M - 2] * kini / (kini * phi[i][M - 2] - phi_out[i][M - 2] + 1)  # c_s[i, M-1]
                    c_l_out[i][M - 1] = c_out[i][M - 2] / (kini * phi[i][M - 2] - phi_out[i][M - 2] + 1)  # c_l[i, M-1]
                    theta_out[i][M - 1] = theta_out[i][M - 2]

class PCR_GPU_Engine_without_next_moving(Simulation):
    ...
    def simulation_loop(self):
        ...
        SolvePullBack[self._gpu_blocks_per_grid_2D, self._gpu_threads_per_block_2D](self._fields_gpu_device,self.user_data["params_GPU"],self._fields_out_gpu_device,xoffs)
        cuda.synchronize()

        TarrUpdate[self._gpu_blocks_per_grid_2D, self._gpu_threads_per_block_2D](self._fields_gpu_device,
                                                                                 self.user_data["params_GPU"],
                                                                                 self._fields_out_gpu_device,
                                                                                 self.time_step_counter,xoffs)
        cuda.synchronize()


        if self.time_step_counter % 10 == 0:
            self.retrieve_fields_from_GPU()
            output_folder = 'pcr_data_6.28_1'
            os.makedirs(output_folder, exist_ok=True)
            phi = self.fields[0].data
            df = pd.DataFrame(phi)
            filename = os.path.join(output_folder,f'RETRIEVE_20_phi_step{self.time_step_counter}.csv')
            df.to_csv(filename, index=False, header=False)

            c = self.fields[1].data
            df = pd.DataFrame(c)
            filename = os.path.join(output_folder,f'RETRIEVE_20_c_step{self.time_step_counter}.csv')
            df.to_csv(filename, index=False, header=False)

            term1 = self.fields[6].data
            df = pd.DataFrame(term1)
            filename = os.path.join(output_folder, f'term1_step{self.time_step_counter}.csv')
            df.to_csv(filename, index=False, header=False)

            term2 = self.fields[7].data
            df = pd.DataFrame(term2)
            filename = os.path.join(output_folder, f'term2_step{self.time_step_counter}.csv')
            df.to_csv(filename, index=False, header=False)

            term3 = self.fields[8].data
            df = pd.DataFrame(term3)
            filename = os.path.join(output_folder, f'term3_step{self.time_step_counter}.csv')
            df.to_csv(filename, index=False, header=False)

            allterm = self.fields[9].data
            df = pd.DataFrame(allterm)
            filename = os.path.join(output_folder, f'allterm_step{self.time_step_counter}.csv')
            df.to_csv(filename, index=False, header=False)


            size = 2000
            Vf = np.sum(self.fields[0].data[1:size, 1:size]) / ((size - 1) * (size - 1))
            Cf = np.sum(self.fields[1].data[1:size, 1:size] / ((size - 1) * (size - 1)))
            print(f"iteration{self.time_step_counter},Vf = {Vf},Cf = {Cf}")
