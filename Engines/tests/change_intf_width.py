...
class change_intf_width(Simulation):
    ...
    def init_fields(self):
        ...
        #init phi
        posArr_h = initXpos * dx + (np.random.rand(N) - 0.5) * 40 * dx # 1D array
        posArr = np.zeros(N)
        posArr[:] = posArr_h
    ...
    def simulation_loop(self):
        ...
        if (self.time_step_counter -1) % 30000 == 0:
            self.retrieve_fields_from_GPU()
            output_folder = 'change_intf_width'  ##
