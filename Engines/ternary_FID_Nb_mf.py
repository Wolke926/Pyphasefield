...
def PullBack(fields,params,fields_out,timestep):
    ...
    stridex, stridey = cuda.gridsize(2)
    M = phi_out.shape[1]
    for i in range(starty + 1, phi_out.shape[0] - 1, stridey):
        for j in range(startx + 1, phi_out.shape[1]- 1, stridex):

            phi_column_j1000 = [phi_out[i][thresh] for i in range(len(phi_out))]
...
class ternary_FID_Nb_mf(Simulation):
    ...
    def simulation_loop(self):
        ...
        PullBack[self._gpu_blocks_per_grid_2D, self._gpu_threads_per_block_2D](self._fields_gpu_device,self.user_data["params_GPU"],self._fields_out_gpu_device,self.time_step_counter -1)
        cuda.synchronize()
        

        tsave = self.user_data["tsave"]
        output_folder = self.user_data["path_save"]

        if (self.time_step_counter -1) % tsave                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    == 0:
            self.retrieve_fields_from_GPU()

            phi = self.fields[0].data
            T = self.fields[7].data
            #find the T interface
            idx_per_row = np.abs(phi - 0.5).argmin(axis=1)
            j_right = int(idx_per_row.max())
            T_int = T[1][j_right]


            txt_file = os.path.join(output_folder, "T_interface_log.txt")
            with open(txt_file, "a") as f:
                f.write(f"{self.time_step_counter - 1}, {T_int}\n")

            # 只保存 phi, c1, c2
            selected_fields = [self.fields[0], self.fields[1], self.fields[4]]
            selected_names  = ['phi', 'c1', 'c2']

            for field, name in zip(selected_fields, selected_names):
