...
@numba.jit
def gibbsliq_Al(T): #?
    return -795.996 + 177.430178 * T - 31.748192 * T * safe_log(T)
@numba.jit #?
def gibbsliq_Nb(T):
    return 21262.202 + 131.229057*T - 26.4711*T * safe_log(T)+0.203475E-3*T**2 - 0.350119E-6*T**3 + 93399/T - 3.06098E-23*T**7
...
@numba.jit
def gibbsBCC_Al(T):
    return -1194.978 + 183.871153 * T - 31.748192 * T * safe_log(T) - 1.231E28 * T**(-9)
@numba.jit
def gibbsBCC_Nb(T): #?
...
def Dgibbsliq_1(X, Y, T, R, VM):
    Z = 1 - X - Y
    D_id = gibbsliq_Al(T) - gibbsliq_Ti(T) + R * T * (math.log(X) - math.log(Z))
...
def DDgibbsBCC_11(X,Y,T,R,VM):
    ...
    DDterm2_11 = 2 * (2*L_Al_Ti_2*(X-Z)+L_Al_Ti_1)*Z + 2*L_Al_Ti_2*X*Z
    DDterm3_11 = 0

    DDex1_11 = DDterm1_11 + DDterm2_11 + DDterm3_11

    DDex2_11 = 2*Y *Z * L_Al_Nb_Ti_0
...
def DDgibbsBCC_12(X,Y,T,R,VM):
    ...
    DDterm1_12 = (-2 * L_Al_Nb_2 * (X - Y) - L_Al_Nb_1) * Y - 2 * L_Al_Nb_2 * X * Y + L_Al_Nb_1 * (X - Y) + X * (
            2 * L_Al_Nb_2 * (X - Y) + L_Al_Nb_1) + L_Al_Nb_2 * (X - Y) ** 2 + L_Al_Nb_0

    DDterm2_12 = 0
    DDterm3_12 = 0
    DDex1_12 = DDterm1_12 + DDterm2_12 + DDterm3_12
    DDex2_12 = Z * (
                L_Al_Nb_Ti_2 * Z + L_Al_Nb_Ti_1 * Y + L_Al_Nb_Ti_0 * X) + L_Al_Nb_Ti_1 * Y * Z + L_Al_Nb_Ti_0 * X * Z
...
def DDgibbsliq_12(X, Y, T, R, VM):
    ...
    DDterm3_12 = 0
    DDex1_12 = DDterm1_12 + DDterm2_12+DDterm3_12
    DDex2_12 =Z*(L_Al_Nb_Ti_2*Z+L_Al_Nb_Ti_1*Y+L_Al_Nb_Ti_0*X)+L_Al_Nb_Ti_1*Y*Z+L_Al_Nb_Ti_0*X*Z
    return (DDid_12 + DDex1_12 + DDex2_12)/VM
@numba.jit
def DDgibbsBCC_13(X, Y, T, R, VM):
    ...
    DDex2_13 =Y*(L_Al_Nb_Ti_2*Z+L_Al_Nb_Ti_1*Y+L_Al_Nb_Ti_0*X)+L_Al_Nb_Ti_2*Y*Z+L_Al_Nb_Ti_0*X*Y
...
def DDgibbsliq_13(X, Y, T, R, VM):
    ...
    DDex1_13 = term1 + term2 + term3
    DDex2_13 = Y * (
                L_Al_Nb_Ti_2 * Z + L_Al_Nb_Ti_1 * Y + L_Al_Nb_Ti_0 * X) + L_Al_Nb_Ti_2 * Y * Z + L_Al_Nb_Ti_0 * X * Y
    return (DDid_13 + DDex1_13 + DDex2_13) / VM
@numba.jit
def DDgibbsBCC_22(X, Y, T, R, VM):
    ...
    L_Al_Nb_Ti_2 = BCC_L_Al_Nb_Ti_2(T)
    DDid_22 = R*T *(1/Y)
    term1 = 2*L_Al_Nb_2*X*Y + 2* X*(-2*L_Al_Nb_2*(X-Y)-L_Al_Nb_1)
    term2 = 0
    term3 = X * L_Nb_Ti_0
    DDex1_22 = term1+ term2+ term3
    DDex2_22 = 2*X*L_Al_Nb_Ti_1* Z
...
def DDgibbsBCC_23(X, Y, T, R, VM):
    ...
    DDex2_23 =X*(L_Al_Nb_Ti_2*Z+L_Al_Nb_Ti_1*Y+L_Al_Nb_Ti_0*X)+L_Al_Nb_Ti_2*X*Z+L_Al_Nb_Ti_1*X*Y
...
def DDgibbsBCC_33(X, Y, T, R, VM):
    ...
    term1 = 0
    term2 = 2*L_Al_Ti_2*X*Z + 2*X*(-2 * L_Al_Ti_2*(X-Z) - L_Al_Ti_1)
    term3 = L_Nb_Ti_0 * Y
    DDex1_33 = term1 + term2 + term3
    DDex2_33 = 2* L_Al_Nb_Ti_2 * Y * X
...
def solveD(fields,params,fields_out):
    ...
    for i in range(starty + 1, phi.shape[0] - 1, stridey):
        for j in range(startx + 1, phi.shape[1] - 1, stridex):
            ...
            Dmiu2_c1 = DDgibbsBCC_12(c1_s[i][j], c2_s[i][j], T, R, VM)
            Dmiu3_c1 = DDgibbsBCC_13(c1_s[i][j], c2_s[i][j], T, R, VM)
            Dmiu1_c3 = DDgibbsBCC_13(c1_s[i][j], c2_s[i][j], T, R, VM)
            Dmiu3_c2 = DDgibbsBCC_23(c1_s[i][j], c2_s[i][j], T, R, VM)
            Dmiu2_c3 = DDgibbsBCC_23(c1_s[i][j], c2_s[i][j], T, R, VM)
...
#----------------------------------------liquid diff----------------------------------------------
            ...
            Dmiu2_c1 = DDgibbsliq_12(c1_l[i][j], c2_l[i][j], T, R, VM)
            Dmiu3_c1 = DDgibbsliq_13(c1_l[i][j], c2_l[i][j], T, R, VM)
            Dmiu1_c3 = DDgibbsliq_13(c1_l[i][j], c2_l[i][j], T, R, VM)
            Dmiu3_c2 = DDgibbsliq_23(c1_l[i][j], c2_l[i][j], T, R, VM)
            Dmiu2_c3 = DDgibbsliq_23(c1_l[i][j], c2_l[i][j], T, R, VM)
