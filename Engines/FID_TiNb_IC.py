...
def solvePhi(fields,params,fields_out):
    ...
    for i in range(starty + 1,phi.shape[0] - 1,stridey):
        for j in range(startx + 1,phi.shape[1] -1,stridex):
            ...
            T = Tarr[i][j]
            f_S =f_s(c_s[i][j], T, R, VM)
            f_L = f_l(c_l[i][j], T, R, VM)
            mu_S = DgibbsBCC_1(c_s[i][j], T, R, VM)
            mu_L = Dgibbsliq_1(c_l[i][j], T, R, VM)
            ...
            if phi_out[i][j] < 0.00001:
                phi_out[i][j] = 0.00001
            elif phi_out[i][j] > 0.9999:
                phi_out[i][j] = 0.9999
...
def solveC(fields,params,fields_out):
    ...
    for i in range(starty + 1, phi.shape[0] - 1, stridey):
        for j in range(startx + 1, phi.shape[1] - 1, stridex):
            ...
            if c_s_out[i][j] < 0.00001:
                c_s_out[i][j] = 0.00001
            elif c_s_out[i][j] > 0.9999:
                c_s_out[i][j] = 0.9999
...
class FID_TiNb_IC(Simulation):
    ...
    def init_fields(self):
        ...
        Tarr = Ts + G * (np.arange(M) * dx - initXpos * dx)  
        Tarr = np.tile(Tarr, (N, 1))  
