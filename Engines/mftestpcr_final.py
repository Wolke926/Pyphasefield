#apply a modified moving frame method
import numpy as np
import os
import pandas as pd
import sympy as sp
...
def TarrUpdate(fields,params,fields_out,timestep):
    ...
    for i in range(starty, Tarr.shape[0], stridey):
        for j in range(startx, Tarr.shape[1], stridex):
            ...
            Tarr_out[i][j] = round((Tarr[i][j] - Vs * dt * G),4)
...
def PullBack(fields,params,fields_out):
    ...
    for i in range(starty + 1, phi.shape[0] - 1, stridey):
        for j in range(startx + 1, phi.shape[1]- 1, stridex):
            ...
            cuda.syncthreads()
@cuda.jit
def transfer_to_temp(fields,params,fields_out,timestep,temp_phi,temp_c,temp_c_l,temp_c_s,temp_theta):
    phi = fields[0]
    Tarr = fields[5]
    phi_out = fields_out[0]
    c_out = fields_out[1]
    ...
    for i in range(starty, phi.shape[0], stridey):
        for j in range(startx, phi.shape[1] - thresh, stridex):

            temp_phi[i][j] = phi_out[i][j + thresh]
            temp_c[i][j] = c_out[i][j + thresh]
            temp_c_l[i][j] = c_l_out[i][j + thresh]
            temp_c_s[i][j] = c_s_out[i][j + thresh]
            temp_theta[i][j] = theta_out[i][j + thresh]
          

@cuda.jit
def PullBack2(fields,params,fields_out,timestep,temp_phi,temp_c,temp_c_l,temp_c_s,temp_theta):
    ...
    for i in range(starty, phi.shape[0], stridey):
        for j in range(startx, phi.shape[1], stridex):
            #Tarr_out[i][j] = T0 + G * ((j + thresh) * dx - Vs * t)
            Tarr_out[i][j] += thresh * dx * G
      
            phi_out[i][j] = temp_phi[i][j]
            theta_out[i][j] = temp_theta[i][j]

            c_out[i][j] = temp_c[i][j]
            c_l_out[i][j] = temp_c_l[i][j]
            c_s_out[i][j] = temp_c_s[i][j]
            

    cuda.syncthreads()


class mftestpcr_final(Simulation):
    ...
    def simulation_loop(self):
        ...
        #phi_column_j1000 = phi[:,thresh].copy_to_host()
        ...
        N = 502
        M = 2002
        temp_phi = cuda.device_array((N, M), dtype=np.float64)
        temp_c = cuda.device_array((N, M), dtype=np.float64)
        temp_c_l = cuda.device_array((N, M), dtype=np.float64)
        temp_c_s = cuda.device_array((N, M), dtype=np.float64)
        temp_theta = cuda.device_array((N, M), dtype=np.float64)
        ...
        if (self.time_step_counter - 1) == 1000000:
            self.retrieve_fields_from_GPU()
            output_folder = 'testmfpcr'  ##
            os.makedirs(output_folder, exist_ok=True)
            phi = self.fields[0].data
            df = pd.DataFrame(phi)
            filename = os.path.join(output_folder, f'phi_before_mf_step_{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            c = self.fields[1].data
            df = pd.DataFrame(c)
            filename = os.path.join(output_folder, f'c_before_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            c_s = self.fields[2].data
            df = pd.DataFrame(c_s)
            filename = os.path.join(output_folder, f'c_s_before_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            c_l = self.fields[3].data
            df = pd.DataFrame(c_l)
            filename = os.path.join(output_folder, f'c_l_before_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            tarr = self.fields[5].data
            df = pd.DataFrame(tarr)
            filename = os.path.join(output_folder, f'tarr_before_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            #apply moving frame
            transfer_to_temp[self._gpu_blocks_per_grid_2D, self._gpu_threads_per_block_2D](self._fields_gpu_device,self.user_data["params_GPU"],self._fields_out_gpu_device,self.time_step_counter -1, temp_phi, temp_c, temp_c_l, temp_c_s, temp_theta)
            PullBack2[self._gpu_blocks_per_grid_2D, self._gpu_threads_per_block_2D](self._fields_gpu_device,self.user_data["params_GPU"],self._fields_out_gpu_device,self.time_step_counter -1,temp_phi, temp_c, temp_c_l, temp_c_s, temp_theta)
            cuda.synchronize()
            ...
            phi = self.fields[0].data
            df = pd.DataFrame(phi)
            filename = os.path.join(output_folder, f'phi_after_mf_step_{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            c = self.fields[1].data
            df = pd.DataFrame(c)
            filename = os.path.join(output_folder, f'c_after_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            c_s = self.fields[2].data
            df = pd.DataFrame(c_s)
            filename = os.path.join(output_folder, f'c_s_after_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            c_l = self.fields[3].data
            df = pd.DataFrame(c_l)
            filename = os.path.join(output_folder, f'c_l_after_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)

            tarr = self.fields[5].data
            df = pd.DataFrame(tarr)
            filename = os.path.join(output_folder, f'tarr_after_mf_step{self.time_step_counter - 1}.csv')
            df.to_csv(filename, index=False, header=False)
