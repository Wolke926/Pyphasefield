...
class old_fid_eng_phi(Simulation):
    ...
    def init_fields(self):
        ...
        for i in range(N):
            ...
            phi[i, :mid - 1] = 0.0       # 左边为 0
            phi[i, mid - 1]  = 0.3       # 过渡点1
            phi[i, mid]      = 0.5       # 过渡点2（中心）
            phi[i, mid + 1]  = 0.7       # 过渡点3
            phi[i, mid + 2:] = 1.0  
...
if (self.time_step_counter - 1) % 100000 == 0:
    ...
    np.save(f"{output_folder}/phi_step_{self.time_step_counter}.npy", phi)
    np.save(f"{output_folder}/c_step_{self.time_step_counter}.npy", c)
    np.save(f"{output_folder}/c_s_step_{self.time_step_counter}.npy", c_s)
    np.save(f"{output_folder}/c_l_step_{self.time_step_counter}.npy", c_l)
    print(f"✅ Saved fields at step {self.time_step_counter}")

# 从 200000 步后开始执行 NaN 检查
if self.time_step_counter >= 200000:
    self.retrieve_fields_from_GPU()
    phi = self.fields[0].data
    c = self.fields[1].data
    c_s = self.fields[2].data
    ...
    if np.isnan(phi).any():
        print(f"⚠️ NaN detected in phi at step {self.time_step_counter}")
        exit(1)
    if np.isnan(c).any():
        print(f"⚠️ NaN detected in c at step {self.time_step_counter}")
        exit(1)
    if np.isnan(c_s).any():
        print(f"⚠️ NaN detected in c_s at step {self.time_step_counter}")
        exit(1)
    if np.isnan(c_l).any():
        print(f"⚠️ NaN detected in c_l at step {self.time_step_counter}")
        exit(1)
