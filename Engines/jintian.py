...
# 尚未定好的参数： miuAb界面能
#注意此处的公式完全按照binary的文章来写，已经发生了改变，“注意”的地方
import numpy as np
import os
import pandas as pd
import sympy as sp
...
def solvePhi(fields,params,fields_out):
    ...
    for i in range(starty + 1,phi.shape[0] - 1,stridey):
        for j in range(startx + 1,phi.shape[1] -1,stridex):
            ...
            #(38)
            phi_l = 1 - phi[i][j]
            sum_1 = (phi[i][j]*mu1_s + phi_l*mu1_l)*(c1_l[i][j]-c1_s[i][j]) #此处和jose一样
            ...
            I_s = laplacian_phi + (math.pi**2/(xi**2)) * (phi[i][j]-0.5) # (17)方括号里面的
            #I_l = laplacian_phi_l + (math.pi**2/(xi**2)) * (1 - phi[i][j])
            #39 [1]
            dphidt = K * (sigma_n * I_s + (math.pi/xi) * math.sqrt(phi[i][j]*phi_l)* dG ) #注意
            curr_phi = phi[i][j]
...
#***************************   c1 calc ***********************************#
            ...
            # part 3 in (17)
            part3_s = phi_out[i][j] * dphidt_ij * (c1_l[i][j] - c1_s[i][j])
            part3_l = (1 - phi_out[i][j]) * dphidt_opp * (c1_s[i][j] - c1_l[i][j]) #注意
...
#************************ calc the c2 ***********************************************************
            ...
            # part 3 in (17)
            part3_s = phi_out[i][j] * dphidt_ij * (c2_l[i][j] - c2_s[i][j])
            part3_l = (1 - phi_out[i][j]) * dphidt_opp * (c2_s[i][j] - c2_l[i][j])
...
class jintian(Simulation):
    ...
    def simulation_loop(self):
        ...
        if self.time_step_counter % 500000                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   == 0:
            ...
            for field, name in zip(self.fields, field_names):
                ...
                filename = os.path.join(output_folder, f'{name}_step_{self.time_step_counter}.csv')
