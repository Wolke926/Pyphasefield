...
#基于ternary_FID,仅仅改变能量计算，其他参数照旧，就是添加0G,能量曲线已经过验证，并且采用miu_tilda
@numba.jit
def gibbsFCC_Fe(T):
    a = -236.7+ 132.416*T - 24.6643*T * math.log(T) - 0.00375752*T**2
    return a
@numba.jit
def gibbsliq_Fe(T):
   a= 13265.87 + 117.57557*T - 23.5143* T* math.log(T)
   return a
@numba.jit
def gibbsliq_Cr(T):
    a= 15483.015 + 146.059775 * T - 26.908 *T * math.log(T) + 0.00189435 * T**2
    return a
@numba.jit
def gibbsFCC_Cr(T):
    a= -1572.94 + 157.643*T - 26.908*T * math.log(T)+ 0.00189435*T**2
    return a
@numba.jit
def gibbsliq(X, Y, T, R, VM):
    # B.1
    gibbs_liq = (R * T/ VM) * (X * math.log(X) + Y * math.log(Y) + (1.0 - X - Y) * math.log(1.0 - X -Y))
    gibbs_liq +=( X* gibbsliq_Cr(T) + Y * gibbsliq_Cr(T) + (1-X-Y) * gibbsliq_Fe(T))/VM
    return gibbs_liq
...
def gibbsFCC(X,Y,T,R,VM,ke_1,ke_2,me_1,me_2,Tn):
    ...
    gibbs_FCC = (R * T / VM) * (X * math.log(X) + Y * math.log(Y) + (1.0 - X - Y) * math.log(1.0 - X -Y))
    gibbs_FCC += ( X* gibbsFCC_Cr(T) + Y * gibbsFCC_Cr(T) + (1-X-Y) * gibbsFCC_Fe(T))/VM
    return gibbs_FCC
...
def Dgibbsliq_1(X,Y,T,R,VM):
    #B.13
    Z = 1 - X - Y
    D_gibbs_liq_1 = (gibbsliq_Cr(T) - gibbsliq_Fe(T) + R * T * (math.log(X) - math.log(Z))) / VM
...
def Dgibbsliq_2(X, Y, T, R, VM):
    Z = 1 - X - Y
    D_gibbs_liq_2 =  (gibbsliq_Cr(T) - gibbsliq_Fe(T) + R * T * (math.log(Y) - math.log(Z))) / VM
...
def DgibbsFCC_1(X,Y,T,R,VM,ke_1, ke_2, me_1, me_2,Tn):
    Z = 1 - X - Y
    return (gibbsFCC_Cr(T) - gibbsFCC_Fe(T) + R * T * (math.log(X) - math.log(Z))) / VM


@numba.jit
def DgibbsFCC_2(X,Y,T,R,VM,ke_1, ke_2, me_1, me_2,Tn):
    Z = 1 - X - Y
    return (gibbsFCC_Cr(T) - gibbsFCC_Fe(T) + R * T * (math.log(Y) - math.log(Z))) / VM
...
class FID_FeCrCr_tilda(Simulation):
    ...
    def just_before_simulating(self):  #in simulation file
        ...
        print("using FID_FeCrCr_tilda kernel")
        params = []
