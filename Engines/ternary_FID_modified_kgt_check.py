...
def gibbsBCC_Ti(T): 
    ...
    #return 26483.26 - 182.426271 * T + 19.0900905 * T * safe_log(T) # bcca2 > 1941
    #return -119924.586 + 638.806871 *T - 87.2182461*T * safe_log(T) #bcta5/cbcca12 > 1941
...
@numba.jit
def gibbsBCC_Nb(T): #?
    return -8519.353 + 142.045474 * T - 26.4711 * T * math.log(T) + 0.203475E-3 * T**2 - 0.35012E-6 * T**3 + 93399/T
...
@numba.jit
def gibbsBCC_Al(T):
    return -1195.361 + 183.871136 * T - 31.748192 * T * safe_log(T) - 1.231E28 * T**(-9)
@numba.jit
def gibbsliq_Al(T): 
    return -795.991 + 177.430209 * T - 31.748192 * T * safe_log(T)
...
def solvePhi(fields,params,fields_out):
    ...
    for i in range(starty + 1,phi.shape[0] - 1,stridey):
        for j in range(startx + 1,phi.shape[1] -1,stridex):
            ...
            #39 [1]
            dphidt = K/N * (sigma_n * (I_s - I_l) + (math.pi ** 2) * dG / (4 * xi)) #此处有不同的表达方式
            curr_phi = phi[i][j]
...
#***************************   c1 calc ***********************************#
            ...
            # part 1 in (17) i = 1, alpha = s
            part1_s = (x1plus_s + x1mins_s + y1plus_s + y1mins_s) / dx #jose一致
            part1_l = (x1plus_l + x1mins_l + y1plus_l + y1mins_l) / dx

            # part 2 in (17) i = 1, alpha = s
            part2_s = P1 * phi_out[i][j] * (1 - phi_out[i][j]) * (mu1_l - mu1_s) #jose一致
            part2_l = P1 * phi_out[i][j] * (1 - phi_out[i][j]) * (mu1_s - mu1_l)

            # part 3 in (17)
            part3_s = phi_out[i][j] * dphidt_ij * (c1_l[i][j] - c1_s[i][j])
            part3_l = (1 - phi_out[i][j]) * dphidt_opp * (c1_s[i][j] - c1_l[i][j]) #一致
...
class ternary_FID_modified_kgt_check(Simulation):
