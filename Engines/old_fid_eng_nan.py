...
def load_fields_from_csv(self, folder_path,add_field_step_offset):
    
    field_names = ["c1", "c2", "c1_s", "c2_s", "c1_l", "c2_l", "phi", "Tarr"]
    
    loaded_fields = {}
    for name in field_names:
        #file_path = os.path.join(folder_path, f"{add_field_step_offset}_end_{name}.csv")
        file_path = os.path.join(folder_path, f"{name}_step_{add_field_step_offset}.csv")
        try:
            loaded_fields[name] = np.loadtxt(file_path, delimiter=",")
            print(f"Loaded {name} from {file_path}, shape: {loaded_fields[name].shape}")
        except Exception as e:
            raise RuntimeError(f"Failed to load {name} from {file_path}: {e}")
    
    return loaded_fields

class old_fid_eng_nan(Simulation):
    ...
    def init_fields(self):
        ...
        add_field_step_offset = self.user_data["add_field_step_offset"]
        if self.user_data["load_from_csv"] == True:
            folder_path = 'find_nan'
            
            fields = load_fields_from_csv(self, folder_path,add_field_step_offset)
            c1 = fields["c1"]
            c2 = fields["c2"]
            c1_s = fields["c1_s"]
            c2_s = fields["c2_s"]
    ...
    def simulation_loop(self):
        ...
        if (self.time_step_counter -1) % 500000                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    == 0:
            ...
            for field, name in zip(self.fields, field_names):
                # 去掉上下左右各2行/列 → 中心区域
                data = field.data[2:rows-2, 2:cols-2]
